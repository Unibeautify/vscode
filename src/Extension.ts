...
export class Extension {
  ...
  createCommands(): Disposable[] {
    const beautifyFileCommand = vscode.commands.registerCommand(
      "Unibeautify.beautifyFile",
      this.beautifyFile.bind(this)
    );
    const beautifySelectionCommand = vscode.commands.registerCommand(
      "Unibeautify.beautifySelection",
      this.beautifySelection.bind(this)
    );
    return [beautifyFileCommand, beautifySelectionCommand];
  }
  ...
  beautifyFile() {
    ...
    unibeautify
      .beautify({ languageName, options, text })
      .then((result: string) => {
